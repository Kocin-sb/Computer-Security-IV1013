...
/**
 *
 * HillDecipher creates an inverted matrix from the key matrix and then decrypts
 * the ciphertext. The program only supports a maximum blocksize of 4, if higher
 * some of the letters becomes decoded wrong, and I really don't know why. All
 * radices are supported; 26, 128, 256.
 *
 */
...
public class HillDecipher {
    ...
    /**
     *
     * Decodes the cipher by creating the inverse to the key matrix and multiply it
     * to the cipher.
     *
     * @param cipher
     * @param keyMatrix
    ...
    public void decode(DenseMatrix<Real> cipher, DenseMatrix<Real> keyMatrix, String plainFile, int radix)
        ...
        for (int i = 0; i < invMatrix.getNumberOfRows(); i++) {
            for (int j = 0; j < invMatrix.getNumberOfColumns(); j++) {
                LargeInteger modNum = LargeInteger.valueOf(invMatrix.get(i, j).longValue())
    ...
    /**
     * Removes the padding from the decrypted message and writes the plaintext to a
     * file as an number representation of the letters.
     *
    ...
    public static void main(String[] args) {
        ...
        if (Integer.parseInt(args[0]) > 256 || Integer.parseInt(args[1]) > 4) {
            System.out.println(
                    "The program only supports a maximum radix of 256 and a maximum blocksize of 8, please try again");
            return;
        }
